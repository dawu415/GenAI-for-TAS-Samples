...
class ModelTokenizedTextChunker(DocTransformer):
    __TEXT_OVERLAP_SENTENCE_LEVEL = -1 # Overlap at a sentence granularity instead of number of characters

    def __init__(self, model_tokenizer_path, token_chunk_size = 128, text_overlap=__TEXT_OVERLAP_SENTENCE_LEVEL):
        try:
    ...
    def __call__(self, parent:Document, texts: List[DocChunk]) -> List[DocChunk]:
        ...
        for text in process_texts:
            ...
            for sent in doc.sents:
                tokenized_sent = self.model_tokenizer(sent.text, truncation=True, return_tensors="np", padding=False, max_length=self.token_chunk_size)["input_ids"]        

                if current_token_chunk_length + tokenized_sent.shape[1] <= self.token_chunk_size:
                    ...
                    chunks.append(DocChunk(content = overlap_text_chunk,
                                           metadata= {**parent.metadata.copy(),"id": chunk_id}
                                          )
                                 )
                    ...
                    overlap_text_chunk += sent.text
                    tokenized_overlap_chunk = self.model_tokenizer(overlap_text_chunk, return_tensors="np", padding=False, truncation=True, max_length=self.token_chunk_size)["input_ids"] 
